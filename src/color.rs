...
impl LuaUserData for Color {
	fn add_methods<'lua, M: LuaUserDataMethods<'lua, Self>>(methods: &mut M) {
		...
		methods.add_meta_function(LuaMetaMethod::Mul, |_, (c1, c2): (Color, Color)| {
			Ok(c1 * c2)
		});

		methods.add_meta_function(LuaMetaMethod::Sub, |_, (c1, c2): (Color, Color)| {
			Ok(c1 - c2)
		});

		methods.add_meta_function(LuaMetaMethod::Div, |_, (c1, c2): (Color, Color)| {
			Ok(c1 / c2)
		});
	}
}
